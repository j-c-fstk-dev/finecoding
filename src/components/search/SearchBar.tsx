...
export function SearchBar() {
    ...
    const fetchSearchIndex = async () => {
      if (!data) {
        ...
        try {
          ...
          setData([]); // Set data to empty array on error to prevent re-fetching
        } finally {
  ...
  const displayedResultsCount = Math.min(postResults.length, MAX_POSTS_IN_DROPDOWN) +
                                Math.min(resourceResults.length, MAX_RESOURCES_IN_DROPDOWN) +
                                Math.min(tagResults.length, MAX_TAGS_IN_DROPDOWN);
      ...
      <Command className="relative overflow-visible bg-transparent">
        ...
        {isOpen && debouncedQuery.length > 0 && (
          ...
          >
            ...
            {totalResults > 0 && totalResults > displayedResultsCount && (
